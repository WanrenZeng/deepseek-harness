...
API keys never enter that collection. The harness resolves a route's key through its own seam before the request reaches pi-ai and passes it as the request's `apiKey` option, which pi-ai treats as the highest-priority auth override; the collection itself still carries only a redacted OAuth credential store for provider-auth operations. Only `github-copilot` is serviceable for OAuth login in this build. Its login snapshots contain public events only (URL, device code, progress, prompt text), prompt answers are accepted at most once per prompt id, and completed/cancelled/failed login sessions are pruned after a short retention window. A route naming no credential resolves as configured-but-keyless and leaves the requirement to the protocol, which is where it actually lives.
...
- **OAuth support is intentionally narrow** — this build serves OAuth login only for `github-copilot`, backed by the harness credential seam for durable token storage and refresh persistence. The configurable-provider directory still withholds OAuth-only catalog providers with no serviceable flow; `openai-codex` remains hidden from the add-provider list, though a stored profile stays editable/removable and `apiKeyEnv` can still authenticate it with a token.
