...
API key 绝不进入该集合。harness 在请求抵达 pi-ai 之前经自身 seam 解析路由密钥，并作为请求的 `apiKey` 选项传入，而 pi-ai 将其视为优先级最高的 auth 覆盖；集合本身只携带用于 provider-auth 的脱敏 OAuth 凭据存储。本构建仅将 `github-copilot` 视为可服务的 OAuth 登录提供方。其登录快照只包含公开事件（URL、设备码、进度、提示文本），同一 prompt id 最多受理一次回答，完成/取消/失败的登录会话会在短保留窗口后清理。没有点名任何凭据的路由会解析为「已配置但无密钥」，把该要求留给协议——那才是它真正所在的位置。
...
- **OAuth 支持刻意收窄**：本构建仅为 `github-copilot` 提供 OAuth 登录，底层使用 harness 凭据 seam 持久化 token 并写回刷新结果。可配置提供方目录仍会隐藏无可服务流程的 OAuth-only catalog 提供方；`openai-codex` 继续不出现在新增列表中，但已存储 profile 仍可编辑/删除，且 `apiKeyEnv` 仍可用 token 认证它。
